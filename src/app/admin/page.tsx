...
function ChartCard({ title, subtitle, icon, children }: { title: string; subtitle: string; icon: React.ReactNode; children: React.ReactNode }) {
	return (
		<motion.div
			...
			animate={{ opacity: 1, y: 0 }}
			className="rounded-[2rem] bg-white/90 backdrop-blur-xl border border-white shadow-[0_18px_60px_rgba(15,23,42,0.08)] p-5"
		>
			...
			<div className="relative h-[180px] w-[180px]">
				<svg viewBox="0 0 100 100" className="h-full w-full -rotate-90">
					{segments.map((segment) => {
						...
						currentAngle += angle;
						return <path key={segment.label} d={path} fill="none" stroke={segment.color} strokeWidth={holeRatio > 0 ? 14 : 38} strokeLinecap="round" />;
					})}
					{holeRatio > 0 ? <circle cx="50" cy="50" r={50 * holeRatio} fill="white" /> : null}
				</svg>
				<div className="absolute inset-0 flex items-center justify-center">
					<div className="text-center">
						<div className="text-2xl font-black text-slate-900">{total}</div>
						<div className="text-xs font-semibold text-slate-500">{innerLabel}</div>
					</div>
				</div>
			</div>
			<div className="grid w-full gap-2">
				{segments.map((segment) => (
					<div key={segment.label} className="flex items-center justify-between rounded-2xl bg-slate-50 px-3 py-2 text-sm">
						<div className="flex items-center gap-2 font-semibold text-slate-700">
							<span className="h-3 w-3 rounded-full" style={{ backgroundColor: segment.color }} />
							{segment.label}
						</div>
						<div className="font-bold text-slate-900">{Math.round((segment.value / total) * 100)}%</div>
					</div>
				))}
			</div>
		</div>

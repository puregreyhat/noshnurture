...
export default function ShoppingListPage() {
  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      doc.text('NoshNurture', 105, 20, { align: 'center' });

      // Subtitle (Light Green)
      doc.setFontSize(12);
      doc.setTextColor(236, 253, 245); // emerald-50
      doc.setFont('helvetica', 'normal');
      doc.text('Your Smart Sustainable Pantry', 105, 28, { align: 'center' });

      // Shopping List Label (Dark Text)
      doc.setFontSize(18);
      doc.setTextColor(40, 40, 40);
      doc.setFont('helvetica', 'bold');
      doc.text('Shopping List', 14, 55);
      ...
      const tableData = items.map(item => [
        ...
        item.added_from === 'low_stock' ? 'Low Stock' : item.added_from === 'recipe' ? 'Recipe' : 'Manual'
      ]);

      autoTable(doc, {
        head: [['Item', 'Quantity', 'Source']],
        body: tableData,
        startY: 70,
        theme: 'grid',
        headStyles: {
      ...
      doc.text('Generated by NoshNurture - Smart Pantry Management', 105, pageHeight - 10, { align: 'center' });

      doc.save('NoshNurture_Shopping_List.pdf');

...
type Step = 'upload' | 'preview' | 'method-select' | 'voice' | 'manual';
...
export default function BillUploadModal({
  ...
  const handleBackClick = () => {
    if (step === 'preview') {
      ...
      setStep('preview');
    } else if (step === 'voice' || step === 'manual') {
      setStep('method-select');
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[600px] flex flex-col">
        ...
        <div className="bg-gradient-to-r from-orange-600 to-yellow-600 text-white p-4 rounded-t-lg flex justify-between items-center">
          <div>
            <h2 className="text-xl font-bold">📄 Bill/Receipt Upload</h2>
                ...
                <ul className="space-y-2">
                  {extractedProducts.map((product, idx) => (
                    <li key={idx} className="text-sm text-gray-600 flex items-start gap-2">
                      ...
                      <span>{product.productName} ({product.quantity} {product.unit}, {product.size})</span>
                    </li>
                  ))}
                </ul>
              </div>

...
export function EditInventoryDialog({ isOpen, onClose, item, onSaved }: Props) {
  ...
  const [expiryDate, setExpiryDate] = useState<string>(item?.expiry_date ? String(item.expiry_date).slice(0,10) : '');
  ...
  useEffect(() => {
    if (item) {
      ...
      setExpiryDate(item.expiry_date ? String(item.expiry_date).slice(0,10) : '');
